...
if(in_array($extension, $extensions) && $size <= $maxSize && $error==0){ $uniqueName=uniqid('', true); //uniqid génère
    //quelque chose comme ca : 5f586bf96dcd38.73540086 $name_image=$uniqueName.".".$extension;
    //$file=5f586bf96dcd38.73540086.jpg move_uploaded_file($tmpName, '../../img/' .$name_image); //$req=$db->
    //prepare('INSERT INTO produits (image) VALUES (?)');
    ...
    //On se connecte à la BDD (base de données)
    ...
    // htmlspecialchars decode — Convertit les entités HTML spéciales en caractères, échappe certains caractères
    //spéciaux comme les chevrons < et>
        $donnees = htmlspecialchars($donnees);
        return $donnees;

        // valid_donnees On échappe les caractères dangereux potentiellement envoyées et effectuer un premier nettoyage
        //des données du formulaire et on place alors le résultat dans nos variables
    ...
    <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
            <div class="px-4 sm:px-0">
                ...
                <p class="mt-1 text-sm text-gray-600">Ces informations seront affichées publiquement, alors
                    faites
                    attention à ce que vous partagez.</p>
            </div>
        </div>
                    ...
                    <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                        <div class="grid grid-cols-3 gap-6">
                            <div class="col-span-3 sm:col-span-2">
                                <label for="nom" class="block text-sm font-medium text-gray-700"> Titre du jeu
                                </label>
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    <input type="text" name="nom" id="nom"
                                        class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300"
                                        placeholder="super mario 3d">
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <textarea id="resume" name="resume" rows="3"
                                    ...
                                    placeholder="Rejoignez Mario, Luigi, Peach et Toad, et partez à l'aventure pour sauver le royaume des Libellas dans Super Mario 3D World + Bowser’s Fury sur Nintendo Switch ! En solo ou avec jusqu'à trois autres joueurs, allez sauver la"></textarea>
                            </div>
                        ...
                        <div>
                            <label for="console" class="block text-sm font-medium text-gray-700"> Console
                            </label>
                            <div class="mt-1">
                                <textarea id="console" name="console" rows="3"
                                    class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md"
                                    placeholder="xbox, playstation, switch, pc"></textarea>
                            </div>
                        </div>
                        <div>
                            <label for="video" class="block text-sm font-medium text-gray-700"> Lien de la video
                            </label>
                            <div class="mt-1">
                                <textarea id="video" name="video" rows="3"
                                    class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md"
                                    placeholder=" https://www.xbox.com/fr-FR/games/store/minecraft-dungeons-luminous-night-adventure-pass/9P226GDNFGGB"></textarea>
                            </div>
                        </div>
                            ...
                            <div class="shadow overflow-hidden sm:rounded-md">
                                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                                    ...
                                    <fieldset>
                                        ...
                                        <div class="text-base font-medium text-gray-900" aria-hidden="true">Type
                                            de
                                            public IMAGE JEU</div>
                                        <div class="mt-4 space-y-4">
                                            <div class="flex items-start">
                                                <div class="flex items-center h-5">
                                                    <input name="categorie" type="radio" value="18"
                                                        class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                                </div>
                                                <div class="ml-3 text-sm">
                                                    <label for="adultes" class="font-medium text-gray-700">Adulte 18
                                                        ans</label>
                                                </div>
                                            </div>
                                            <div class="flex items-start">
                                                <div class="flex items-center h-5">
                                                    <input name="categorie" type="radio" value="16"
                                                        class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                                </div>
                                                <div class="ml-3 text-sm">
                                                    <label for="adultes" class="font-medium text-gray-700">A
                                                        partir de
                                                        16 ans</label>
                                                </div>
                                            </div>
                                            <div class="flex items-start">
                                                <div class="flex items-center h-5">
                                                    <input name="categorie" type="radio" value="12"
                                                        class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                                </div>
                                                <div class="ml-3 text-sm">
                                                    <label for="enfants" class="font-medium text-gray-700">A
                                                        partir de
                                                        12 ans</label>
                                                </div>
                                            </div>
                                            <div class="flex items-start">
                                                <div class="flex items-center h-5">
                                                    <input name="categorie" type="radio" value="7"
                                                        class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                                </div>
                                                <div class="ml-3 text-sm">
                                                    <label for="enfants" class="font-medium text-gray-700">A
                                            ...
                                            <div>
                                                <input type="file" id="file" name="produits"
                                                    accept="image/png, image/jpeg, image/jfif, image/jpg" />
                                                <img id="add-img" src="./icons8-ajouter-une-image-90.png"
